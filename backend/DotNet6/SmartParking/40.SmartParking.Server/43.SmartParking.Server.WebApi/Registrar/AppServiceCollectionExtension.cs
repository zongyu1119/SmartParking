...
namespace SmartParking.Server.WebApi.Registrar
{
    ...
    public static class AppServiceCollectionExtension
    {
        ...
        private static void RegisterServiceByAttribute(this IServiceCollection services, ServiceLifetime serviceLifetime, List<Assembly> allAssembly)
        {
            ...
            List<Type> types = allAssembly.SelectMany(t => t.GetTypes()).Where(t => 
            t.GetCustomAttributes(typeof(AppServiceAttribute), false).Length > 0 && t.GetCustomAttribute<AppServiceAttribute>()?.Lifetime == serviceLifetime && t.IsClass && !t.IsAbstract).ToList();
